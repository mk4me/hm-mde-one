...
const FCDEffectParameter* FCDEffectParameterList::FindReference(const char* reference) const
{
	...
	{
		if (IsEquivalent((*it)->GetReference(), reference)) return (*it);
	}
	return NULL;
}
...
{
	// Only search for strings that are possible - if a not-possible reference
	// is set, it will be converted automagically using the same function
	const fm::string& validId = FCDObjectWithId::CleanSubId(reference);

	for (FCDEffectParameterTrackList::iterator it = begin(); it != end(); ++it)
	{
		if ((*it)->GetReference() == validId) list.AddParameter(*it);
	}
}
