...
bool ReaderWriterPlugin::pushSkeletonToOsg(SFMesh* fmesh, ref_ptr<osg::Group> root) const
{
	...
	if (fmesh->skeleton && skeleton->bones_count > 0)
	{
		...
		ref_ptr<Group> root_bone = new CSkeletonNode();
		int roots = 0;
		...
		{	
			...
			{
				skeleton->bones[b].node = new CSkeletonNode();
			...
			// id
			((CSkeletonNode*)skeleton->bones[b].node.get())->setId(b);
		}
...
bool ReaderWriterPlugin::createArrayHoldingBones	
	(std::vector<CSkeletonNode*>* bones, ref_ptr<osg::Group> root) const
{
	CSkeletonNode* skeleteon = NULL;

	// find root bone
	int childNum = root->getNumChildren();
	for (int i = 0; i < childNum; ++i)
	{		
		skeleteon = dynamic_cast<CSkeletonNode*>(root->getChild(i));
		if (skeleteon)
			break;
	}
	...
	struct Obj
	{
		static void createArrayOfBones(CSkeletonNode* bone, 
			vector<CSkeletonNode*>* bones)
		{
			bones->push_back(bone);
			for (unsigned int b = 0; b < bone->getNumChildren(); ++b)
				createArrayOfBones((CSkeletonNode*)bone->getChild(b), bones);
		}
	};
...
bool ReaderWriterPlugin::pushSkeletalAnimationToOSG	
	(wstring* name, SSkeletalAnimation* anim, std::vector<CSkeletonNode*>* bones)	const
{
    ...
    // R.Z. Moze sie zdazyc tak i z animacja rozpocznie sie od pewnego momentu - wtedy dlugosc poczatkowych wezlow bedzie rowna zera - moze sie rozsypac program. 
...
osgDB::ReaderWriter::ReadResult ReaderWriterPlugin::readNode(const std::string& file, const osgDB::ReaderWriter::Options* options) const
{
 ...
 //       19.04.2010 R.Zowl - poprzednio by³o to co ponizej zakomentowane.  - testy dla ³adowania modelu bez mesha
