...
const QStringList CommServ::sessionsHeader = QStringList() << "ID" << "Motion kind ID" << "Date" << "Description";

const QStringList CommServ::trialHeader = QStringList() << "ID" << "Description" << "Duration";

const QStringList CommServ::fileHeader = QStringList() << "ID" << "Description" << "Name";

CommServ::CommServ(QWidget* parent) : QDialog(parent)
{
	resize(600, height());
	//UI
	...
	setTableView(sessionTable, CommServ::sessionsHeader);
	//trial table
	trialTable = new QTableWidget();
	setTableView(trialTable, CommServ::trialHeader);
	//file table
	fileTable = new QTableWidget();
	setTableView(fileTable, CommServ::fileHeader);
...
//	tabbar->setMask(QPixmap::grabWidget(tabs->tabBar(), 0, 0, -1, -1));
	...
	setWindowTitle("QT CommServ");
	//settings
	commServ = EDRCM::CommunicationService::createInstanceFirstTime();
	commServ->init();
	transportManager = new EDRCM::TransportWSDL_FTPS();
	queryManager = new EDRCM::QueryWSDL();
	commServ->setTransportManager(transportManager);
	commServ->setQueryManager(queryManager);
	readSettings();
	setChangedSettings();
	sessions = new std::vector<EDRCM::Session>();
	trials = new std::vector<EDRCM::Trial>();
	files = new std::vector<EDRCM::File>();
}
...
void CommServ::sessionButtonClicked()
{
	getSessions();
}

void CommServ::setChangedSettings()
{
...
void CommServ::writeSettings()
{
	QSettings settings("Marek Daniluk", "CommServ");
...
void CommServ::getSessions()
{
	try
	{
		...
		const std::vector<EDRCM::Session>* ss = commServ->listLabSessionsWithAttributes(0); //listLabSessionsWithAttributes(1);
		sessions->clear();
		trials->clear();
		files->clear();
	...
	catch(EDRCM::EDRException& exc)
	{
		QMessageBox::warning(this, tr("Error"), tr(exc.getExceptionMessage().c_str()));
		EDRCM::EDRException::log(exc.getExceptionMessage());
	}
...
//
//void CommServ::SessionDetailsWithAttributesParser(const std::string& data)
//{
...
//}
//
//void CommServ::parseSession(const QDomElement &element)
//{
//	EDRCM::Session s;
...
void CommServ::setTableView(QTableWidget* table, const QStringList& header)
{
	table->clear();
...
bool CommServ::validateQuerySettings()
{
	QSettings settings("Marek Daniluk", "CommServ");
...
void CommServ::getSessionTrials()
{
	if(sessionTable->currentRow() != -1)
	{
		...
		try
		{
			setTableView(trialTable, CommServ::trialHeader);
			setTableView(fileTable, CommServ::fileHeader);
			const std::vector<EDRCM::Trial>* sessionTrials = commServ->listSessionTrials(sessionTable->item(sessionTable->currentRow(), 0)->text().toInt());
			trials->clear();
			files->clear();
...
void CommServ::getTrialFiles()
{
	if(sessionTable->currentRow() != -1)
	{
		...
		try
		{
			...
			const std::vector<EDRCM::File>* trialFiles = commServ->listFiles(id, "trial");
			files->clear();
...
void CommServ::downloadSelected()
{
...
void CommServ::selectSession(int row, int column)
{
	tabs->setCurrentIndex(1);
	getSessionTrials();
}

void CommServ::selectTrial(int row, int column)
{
	tabs->setCurrentIndex(2);
	getTrialFiles();
}

void CommServ::selectFile(int row, int column)
{
	downloadSelected();
}
