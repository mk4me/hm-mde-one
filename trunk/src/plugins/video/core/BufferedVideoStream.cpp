...
AVOVERRIDE double BufferedVideoStream::getTime() const
{
  VM_FUNCTION_PROLOG;
...
AVOVERRIDE double BufferedVideoStream::getFrameTimestamp() const
{
  VM_FUNCTION_PROLOG;
...
AVOVERRIDE double BufferedVideoStream::getFrameEndTimestamp() const
{
  VM_FUNCTION_PROLOG;
  double timestamp = 0.0;
...
AVOVERRIDE bool BufferedVideoStream::setTime( double time )
{
  VM_FUNCTION_PROLOG;
  ...
  if ( !convertedFrames->getFrame(time) ) {
    ...
    if ( const Picture * pending = pendingFrames->getFrame(time, &startTime, &endTime) ) {
      ...
      endTime = innerStream->getFrameEndTimestamp();
...
AVOVERRIDE bool BufferedVideoStream::getData( Picture & dst )
{
  VM_FUNCTION_PROLOG;
...
AVOVERRIDE bool BufferedVideoStream::readNext()
{
  return false;
}
