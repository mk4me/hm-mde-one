...
namespace COLLADAFW
{
	...
	public:
		...
		virtual bool loadDocument(const String& fileName, IWriter* writer, bool parseDocument)=0;
		...
		/** Starts loading the model and feeds the writer with data.
         ...
         @param writer The writer that should be fed with data.
         @return True, if loading succeeded, false otherwise.*/
		virtual bool loadDocument(const String& uri, const char* buffer, int length, IWriter* writer, bool parseDocument)=0;
