...
bool MultiViewCameraItem::isVisible() const
{
    if ( parent->getChildIndex(camera) != parent->getNumChildren() ) {
        // gdy mamy swticha dziecko może być wyłączone
        if ( osg::Switch* sw = parent->asSwitch() ) {
            // dziecko może być wyłączone
            return sw->getChildValue(camera);
        } else {
            // gdy jest zwykła grupa na pewno dzieko jest widoczne
            return true;
        }
    }
    return false;
...
void MultiViewCameraItem::setVisible( bool visible )
{
    if ( visible ) {
        if ( !isVisible() ) {
            if ( childIdx < 0 ) {
                // rozpoczynał jako niewidoczny
                parent->addChild(camera);
            } else {
                // wcześniej wyłączyliśmy
                if ( parent->asSwitch() ) {
