...
{
    //! Okno względem którego sprawdzać widoczność.
    osg::observer_ptr<osgWidget::Window> window;
    //! Widget dla którego dostosowywać współrzędne tekstury.
    ...
    virtual void onImageResized(vidlib::VideoImageStreamSizeOptimizer* optimizer, int prevS, int prevT)
    {
        ...
        if ( osg::StateSet* state = widget->getStateSet() ) {
            if ( osg::TextureRectangle* rect = dynamic_cast<osg::TextureRectangle*>(state->getTextureAttribute(0, osg::StateAttribute::TEXTURE)) ) {
                // przeskalowanie współrzędnych tekstury
...
format(PixelFormatBGRA)
{
    ...
    // TIP: w przykładach Qt nie ma jawnego zwalniania akcji
...
void VideoWidget::createScene()
{
    ...
    // na wszelki wypadek czyścimy scenę
    clearScene();

    // stworzenie widgetów i dodanie ich do multi widoku
    BOOST_FOREACH(osg::Image* image, images) {
        // wyliczenie współczynnika proporcji oraz aktualizacja zbiorczego współczynnika
...
void VideoWidget::clearScene()
{
    ...
    // reinicjalizacja multi view (po usunięciu wszystkich węzłów - wymagana)
