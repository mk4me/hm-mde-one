...
{
	//Aktualizuje główną ścieżkę danych
	localUsersDataPath = usersDataPath;

	//Muszę aktualizować ścieżki
	if(user_.id() != -1){
		rebuildUserPaths();
	}
}
...
{
	//TODO
	//dodać coś sensownie mieszającego
		...
		if(std::isalnum(c) == false){
			...
			if(first == true){
				...
				case 2:
					...
					//obcinamy do dyżych liter
					nChar = 'A' + c % 24;
					break;

				case 0:

					//obcinamy do małych liter
					nChar = 'a' + c % 24;
					break;
				}
			}
...
void DataSourcePathsManager::rebuildUserPaths()
{
	...
	// dodaję na początek m dla unikalności (od motion) oraz s na koniec (od shallow)
	localMotionShallowCopyPath = localMotionSchemasPath / ("m" + userHash_ + "s.xml");
	// dodaję na początek m dla unikalności (od motion) oraz m na koniec (od metadata)
	localMotionMetadataPath = localMotionSchemasPath / ("m" + userHash_ + "m.xml");
	// dodaję na początek med dla unikalności (od medical) oraz s na koniec (od shallow)
...
{
	// próbujemy posprzątać po użytkowniku jego dane usuwając ich roota
	try{
