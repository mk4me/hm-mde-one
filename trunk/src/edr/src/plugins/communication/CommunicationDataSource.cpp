...
PatientItem * MedicalView::createPatientItemTree(const communication::MedicalShallowCopy::Patient * patient)
{
    ...
    if(groupedByDateSessions.size() == 2){
        ...
        //tworzymy grupy badań - przed i po zabiegu
...
{    
    if(motionShallowCopy != nullptr && medicalShallowCopy != nullptr){
        ...
        //budujemy drzewo i tabelę pacjentów na potrzeby kart
...
{
    if(widget == nullptr){
        //inicjalizujemy zakłądki naszego okienka
        widget = new QWidget();
...
void CommunicationDataSource::init(core::IMemoryDataManager * memoryDataManager, core::IFileDataManager * fileDataManager)
{
    ...
    if(requests.empty() == false){
        //dorzucamy pakiet zleceń dla CM
        //na zakońćzenie musi odświeżyć ui
        communication::CommunicationManager::CompleteRequest req;
        ...
        //parsujemy pliki i odświeżamy schematy
        ...
        //mamy dane - ściągamy zdjęcia pacjentów
...
bool CommunicationDataSource::downloadFiles(const std::vector<const communication::MotionShallowCopy::File *> & files)
{
    bool ret = true;
    if(files.size() > 10){
        //warning - sporo plików, czy kontynuować
        QString info;
    ...
    //utorzone ścieżki dla rollbacka jeśli coś się nie uda
    ...
    //przygotowujemy pliki do ściągania w formie transakcji która automatycznie się rollbackuje
    for(auto it = files.begin(); it != files.end(); it++){
