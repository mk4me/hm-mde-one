...
SegmentsDescriptors BvhSerie::generateConnections(const kinematic::Skeleton& skeleton, const IDMap& idmap)
{
	SegmentsDescriptors sd;
	typedef kinematic::JointConstPtr Joint;
...
BvhSerie::IDMap BvhSerie::generateIds(const kinematic::Skeleton& skeleton)
{
	typedef kinematic::JointConstPtr Joint;
	IDMap ids;
...
osg::Quat BvhSerie::getJointCurrentLocalRotation(kinematic::JointConstPtr joint)
{
	...
	for (auto state : frame) {
		if (state->name == joint->name) {
            ...
            double mul = bvhData->first->getAngle() == SkeletalModel::radians ? 1.0 : osg::DegreesToRadians(1.0);
...
osg::Vec3 BvhSerie::getCurrentRootPosition(kinematic::JointConstPtr root)
{
