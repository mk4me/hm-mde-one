...
bool Chart::addSerie( const ChartSeriePtr& serie, const std::string & name )
{
    if ( geode->containsDrawable(serie) ) {
        UTILS_FAIL("Kanał już jest dodany.");
        return false;
    } else {
...
void Chart::repositionAll()
{
    ...
    // musimy też uwzględnić prawy opis wartości na osi X
    gridPosition.w() = w - (wDelta + xDelta);

    gridPosition.y() = y + yDelta;

    // musimy tez uwzględnić wysokośc tekstu górnej wartości na osi Y
...
//! Przed wywołaniem tej metody labelki powinny być zaktualizowane!!
...
void Chart::setGridDensity(int gridDensity)
{
    ...
    // +1 ponieważ dodawane na ekstremum
...
{
    //odśwież labelki jeśli to konieczne
...
void Chart::refreshAll()
{
    ...
    //sprawdz czy jest sens odświeżać
    if(gridPosition.w() <= 0 || gridPosition.z() <= 0)
    {
        return;
    }

    // odstęp miedzy elementami;
    // 2 podane jawnie, bo trzeba zarezerwować współrzędne dla tła i osiek
    float zStep = zRange/float(3 + series.size());
    float currentZ = z;
...
void Chart::formatStream(std::ostringstream& stream, float min, float max, int decimalsToShow /*= 2*/ )
{
    ...
    // rząd różnicy między max i min (ujemny dla diff < 1)
...
{
    // czy w ogóle jest coś do pkazania?
    if ( condition ) {
...
{
    // czy w ogóle jest coś do pokazania?
    if ( condition ) {
...
void Chart::refreshSeries(float & currentZ, float zStep)
{
    ...
    if ( normalized ) {
        ...
        BOOST_FOREACH( const ChartSeriePtr& serie, series ) {
            ...
            // uzupełnienie min/max zakresu danej jednostki
            ...
            // uzupełnienie podziału po jednostkach
            byUnits.insert(std::make_pair(serie->getYUnit(), serie));
        }

        if ( shareZero ) {
            // uwspólnienie polega na przeskalowaniu zakresów tak, żeby względna odległość min/max od zera
            // była jednakowa
            ...
            // normalizujemy zakres - w takim stosunku do zakresu muszą być min i max
            ...
            if ( maxDiff != 0.0f ) {
                ...
                // pomocnicze zmienne pozwalające uniknąć wielokrotnego dzielenia
                float scaleMinToMaxFactor = maxPosRatio / maxNegRatio;
                float scaleMaxToMinFactor = 1.0f / scaleMinToMaxFactor;
                // skalujemy wszystkie zakresy tak, aby stosunek max i min do zakresu był taki jak wyliczone zbiorcze
                for ( auto it = unitInfo.begin(), last = unitInfo.end(); it != last; ++it ) {
                    std::pair<float, float>& range = it->second;
                    float diff = range.second - range.first;
                    if ( diff != 0.0f ) {
                        ...
                        if ( maxNegRatio == 0.0f ) {
                            ...
                            // rozciągamy górny zakres
                            range.second = scaleMinToMaxFactor * fabs(range.first);
                        } else {
                            // rozciągamy dolny zakres
                            range.first = -scaleMaxToMinFactor * fabs(range.second);
                        }
                    } else {
                        // ponieważ funkcja jest niezmienna wporawdzamy rozszerzamy sztucznie zakres
                        range.first -= maxNegRatio;
                        range.second += maxPosRatio;
                    }
                }
            } else {
                // ponieważ wszystkie funkcje są stałe wprowadzamy sztuczny zakres
                for ( auto it = unitInfo.begin(), last = unitInfo.end(); it != last; ++it ) {
                    std::pair<float, float>& range = it->second;
                    // ponieważ funkcja jest niezmienna wporawdzamy rozszerzamy sztucznie zakres
                    range.first -= 1.0f;
        ...
        for ( auto unit = unitInfo.begin(); unit != unitInfo.end(); ++unit ) {
            ...
            // określamy minimum i maksimum z danego zakresu
            float xMin = FLT_MAX;
            float xMax = -FLT_MAX;
...
void Chart::refreshLabels( float z )
{
    ...
    if ( activeSerie ) {
        ...
        float ny = (pos.y() - y) / h;
        float nh = size.y() / h;
        // przełożenie znormalizowanych współrzędnych na zakres jednostek
...
void Chart::refreshCursor( float z, float range )
{
    ...
    if ( cursorTextCond && cursorCond) {
        // określamy miejsce gdzie będziemy rysować tekst
        ...
        //ustalamy pozycję - mamy punkt zaczepienia (cursorX, cursorY)
        //defaultowo wartości sa pokazywane z prawej storny kursora
        //ale może się okazać że będą niewidoczne więc będziemy musieli je modyfikować
