...
namespace hmdbServices
{
	...
	HMDBSERVICES_EXPORT const DateTime toTime(const std::string & time);
	//! \param time Czas któy konwertujemy do ciągu znaków w formacie UTC
	HMDBSERVICES_EXPORT const std::string toString(const struct tm & time);
	//! \param time Czas któy konwertujemy do ciągu znaków w formacie UTC
	HMDBSERVICES_EXPORT const std::string toString(const DateTime & time);
}
