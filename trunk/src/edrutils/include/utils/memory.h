...
namespace utils
{
	...
	public:
		...
		template< class Y >
		bool owner_before(const MainSpace::weak_ptr<Y>& other) const { return impl.owner_before(other); }

		template< class Y >
		bool owner_before(const MainSpace::shared_ptr<Y>& other) const { return impl.owner_before(other); }
	...
	std::basic_ostream<U, V>& operator<<(std::basic_ostream<U, V>& os, const shared_ptr_void& ptr);
	...
	{	// class for reference counted resource management
		...
		template<typename U>
		friend bool operator==(const shared_ptr_void& lhs, const MainSpace::shared_ptr<U>& rhs);
		template<typename U>
		friend bool operator!=(const shared_ptr_void& lhs, const MainSpace::shared_ptr<U>& rhs);
		template<typename U>
		friend bool operator<(const shared_ptr_void& lhs, const MainSpace::shared_ptr<U>& rhs);
		...
		friend bool operator>=(const shared_ptr_void& lhs, const MainSpace::shared_ptr<U>& rhs);
		template<typename U>
		friend bool operator==(const MainSpace::shared_ptr<U>& lhs, const shared_ptr_void& rhs);
		template<typename U>
		friend bool operator!=(const MainSpace::shared_ptr<U>& lhs, const shared_ptr_void& rhs);
		template<typename U>
		friend bool operator<(const MainSpace::shared_ptr<U>& lhs, const shared_ptr_void& rhs);
		...
		friend bool operator>=(std::nullptr_t lhs, const shared_ptr_void& rhs);
		template <class U, class V>
		friend std::basic_ostream<U, V>& operator<<(std::basic_ostream<U, V>& os, const shared_ptr_void& ptr);
		...
		friend shared_ptr_void atomic_load_explicit(const shared_ptr_void* p,
			std::memory_order mo);
		friend void atomic_store(shared_ptr_void* p,
			shared_ptr_void r);
		friend void atomic_store_explicit(shared_ptr_void* p,
			shared_ptr_void r,
			std::memory_order mo);
	...
	private:
		...
		public:
			...
			InnerDeleter(const T & deleter) : deleter_(deleter) {}
			~InnerDeleter() {}
	...
	public:
		...
		template<class Deleter >
		shared_ptr_void(void* ptr, Deleter d) : impl(static_cast<char*>(ptr), InnerDeleter<Deleter>(d)) {}

		template<class Deleter, class Alloc >
		shared_ptr_void(void* ptr, Deleter d, Alloc alloc) : impl(static_cast<char*>(ptr), InnerDeleter<Deleter>(d), alloc) {}

		template< class Deleter >
		shared_ptr_void(nullptr_t, Deleter d) : impl(nullptr_t, InnerDeleter<Deleter>(d)) {}

		template< class Deleter, class Alloc >
		shared_ptr_void(nullptr_t, Deleter d, Alloc alloc) : impl(nullptr_t, InnerDeleter<Deleter>(d), alloc) {}
		...
		template<class Deleter >
		shared_ptr_void(MainSpace::unique_ptr<void, Deleter>&& r) : impl(static_cast<char*>(r.release()), InnerDeleter<decltype(r.get_deleter())>(r.get_deleter())) {}
		...
		template<class Deleter>
		shared_ptr_void& operator=(MainSpace::unique_ptr<void, Deleter>&& r) { *this = shared_ptr_void(r.release(), r.get_deleter()); return *this; }

		void reset();
		//void reset(void* ptr) { impl.reset(ptr, *get_deleter(impl)); }
		template<class Deleter>
		void reset(void* ptr, Deleter d) { impl.reset(static_cast<char*>(ptr), InnerDeleter<Deleter> (d)); }
		template<class Deleter, class Alloc >
		void reset(void* ptr, Deleter d, Alloc alloc){ impl.reset(static_cast<char*>(ptr), InnerDeleter<Deleter> (d), alloc); }
	...
	Deleter* get_deleter(const shared_ptr_void& p)
	{
		...
		auto deleter = get_deleter<InnerDeleter<Deleter>>(p.impl);
		if (deleter != nullptr){
			ret = deleter->deleter();
		}
