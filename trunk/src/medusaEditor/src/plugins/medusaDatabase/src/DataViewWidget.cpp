...
#include <plugins/medusaDatabase/IDataSourceContent.h>
#include <plugins/medusaDatabase/IDataSourcePerspective.h>
#include <plugins/medusaDatabase/IDataSourceContent.h>
#include <plugins/medusaDatabase/IHMDBShallowCopyContext.h>
#include <plugins/medusaDatabase/DataSourcePerspective.h>
#include <plugins/medusaDatabase/DataSourceDefaultContent.h>
#include <plugins/medusaDatabase/IHMDBShallowCopyContext.h>
...
{
public:
	OperationImpl(const QString & name, medusaDatabase::IHMDBRemoteContext::OperationPtr op)
		: op(op), name_(name)
	{
...
private:
	QString name_;
	medusaDatabase::IHMDBRemoteContext::OperationPtr op;
};

bool mapContentTypeToDataType(const medusaDatabase::ContentType ct,
	medusaDatabase::DataType & dt)
{
	bool ret = true;

	switch (ct)
	{
	case medusaDatabase::PatientContent:
		dt = medusaDatabase::PatientType;
		break;

	case medusaDatabase::SubjectContent:
		dt = medusaDatabase::SubjectType;
		break;

	case medusaDatabase::SessionContent:
		dt = medusaDatabase::SessionType;
		break;

	case medusaDatabase::MotionContent:
		dt = medusaDatabase::MotionType;
		break;

	case medusaDatabase::FileContent:
		dt = medusaDatabase::FileType;
		break;
...
public:
	...
	{
		qRegisterMetaType<medusaDatabase::IDataSourcePerspective*>("medusaDatabase::IDataSourcePerspective*");
		qRegisterMetaType<medusaDatabase::IDataSourceContent*>("medusaDatabase::IDataSourceContent*");
		qRegisterMetaType<medusaDatabase::ShallowCopyFilter*>("medusaDatabase::ShallowCopyFilter*");
		qRegisterMetaType<hmdbServices::ID>("hmdbServices::ID");
		qRegisterMetaType<medusaDatabase::DataType>("medusaDatabase::DataType");
	}
};
...
QIcon DataViewWidget::statusIcon(const DataStatus dataStatus)
{
	...
	case DataStatus::Local:
		s = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/lokalne.png"));
		break;

	default:
	case DataStatus::Remote:
		s = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/zdalne.png"));
		break;
	case DataStatus::PartiallyLocal:
		s = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/pol-na-pol-green.png"));
		break;
	}
	...
	case DataStatus::Loaded:
		u = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/zaladowane.png"));
		break;

	default:
	case DataStatus::Unloaded:
		u = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/niezaladowane.png"));
		break;
	case DataStatus::PartiallyLoaded:
		u = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/polnapol-ladowanie2.png"));
		break;
	}
	...
	if (shallowCopyContext_->shallowCopyRemoteContext() != nullptr){
		...
		case DataStatus::Valid:
			v = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/zaladowane.png"));
			break;

		default:
		case DataStatus::Outdated:
			v = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/niezaladowane.png"));
			break;
		case DataStatus::PartiallyValid:
			v = QPixmap(QString::fromUtf8(":/medusaDatabase/icons/polnapol-ladowanie2.png"));
			break;
		}
...
DataViewWidget::DataViewWidget(medusaDatabase::IHMDBShallowCopyContextPtr shallowCopyContext,
...
medusaDatabase::ShallowCopyConstPtr DataViewWidget::currentShallowCopy() const
{
	return currentShallowCopy_;
}

medusaDatabase::ShallowCopyConstPtr DataViewWidget::completeShallowCopy() const
{
...
void DataViewWidget::extractLocalShallowCopyAndTryUpdate()
{
	...
	if (sc != nullptr && mq != nullptr){
		...
		if (medusaDatabase::ShallowCopyUtils::shallowCopyRequiresRefresh(*sc, dbTime) == true){
...
medusaDatabase::DataStatus DataViewWidget::refreshDataStatus(QTreeWidgetItem * item,
...
{	
	...
	medusaDatabase::DataStatus status;
	...
	if (childrenCount > 0){
		...
		auto contentItem = dynamic_cast<const medusaDatabase::TreeWidgetContentItem*>(item);
...
medusaDatabase::DataStatus DataViewWidget::refrshItemContent(QTreeWidgetItem * item,
	const bool perspectiveFirst)
{	
	int statusColumn = -1;
	QStringList headers;
	...
	if (childrenCount > 0){
		...
		if (contentItem != nullptr){
			const auto id = contentItem->id();
			//wystarczy switch + dynamic_cast
			DataType dt = medusaDatabase::FileType;
			if (mapContentTypeToDataType(contentItem->contentType(), dt) == true){
				status = shallowCopyContext_->shallowCopyDataContext()->dataStatusManager()->transaction()->dataStatus(dt, id);
			}
		}
	...
	item->setHidden(status.storage() == medusaDatabase::DataStatus::Remote);
#endif
#endif
...
void DataViewWidget::setPerspective(medusaDatabase::IDataSourcePerspective * perspective)
{
	perspective_ = perspective;
	rebuildPerspective();
}

void DataViewWidget::setContent(medusaDatabase::IDataSourceContent * content)
{
	content_ = content;
	refreshContent();
}

void filterShallowCopy(const medusaDatabase::ShallowCopy & inShallow,
	medusaDatabase::ShallowCopy & outShallow,
	const medusaDatabase::ShallowCopyFilter * filter)
{
...
void DataViewWidget::setFilter(medusaDatabase::ShallowCopyFilter * filter)
{
	filter_ = filter;
	setShallowCopy(completeShallowCopy_);
}

void DataViewWidget::setShallowCopy(const medusaDatabase::ShallowCopyConstPtr shallowCopy)
{	
...
{
	medusaDatabase::ShallowCopyPtr newShallowCopy(new medusaDatabase::ShallowCopy);
...
void dataItems(const medusaDatabase::TreeWidgetContentItem * contentItem, std::map<medusaDatabase::DataType, std::set<hmdbServices::ID>> & dItems)
{
	using namespace medusaDatabase;

	medusaDatabase::DataType dt = medusaDatabase::FileType;
...
void DataViewWidget::onLoad()
{
	...
	if (sItems.isEmpty() == false){
		...
		auto contentItem = dynamic_cast<medusaDatabase::TreeWidgetContentItem*>(sItem);
		...
		if (contentItem != nullptr){
			...
			try{
				...
				std::map<medusaDatabase::DataType, std::set<hmdbServices::ID>> items;
...
void execOperation(medusaDatabase::IHMDBRemoteContext::OperationPtr operation, utils::shared_ptr<coreUI::CoreCursorChanger> cursorChanger)
{
	operation->start();
	operation->wait();
...
void DataViewWidget::synchronize(medusaDatabase::IHMDBShallowCopyRemoteContext::SynchronizeOperationPtr sOp,
...
{	
	...
	medusaDatabase::StorageFileNames mod = filterModifiedTrialFiles(sOp->incrementalBranchShallowCopy());
	if (!mod.empty()) {
...
void DataViewWidget::onTrySynchronize()
{
	if (shallowCopyContext_->shallowCopyRemoteContext() != nullptr){
		...
		if (currentShallowCopy_ != nullptr){
			...
			if (mq != nullptr){
				const auto dbTime = mq->dataModificationTime();
				synch = medusaDatabase::ShallowCopyUtils::shallowCopyRequiresRefresh(*currentShallowCopy_, dbTime);
			}
		}
...
medusaDatabase::StorageFileNames extractItemFiles(const QTreeWidgetItem* item,
	const medusaDatabase::ShallowCopy & shalowCopy)
{
	medusaDatabase::StorageFileNames ret;

	bool found = false;
	const auto contentItem = dynamic_cast<const medusaDatabase::TreeWidgetContentItem*>(item);
	if (contentItem != nullptr){
...
void DataViewWidget::download(medusaDatabase::IHMDBRemoteContext::OperationPtr dOp,
	utils::shared_ptr<coreUI::CoreCursorChanger> cursorChanger)
{
	dOp->start();
	dOp->wait();
...
void DataViewWidget::setupDownload(const medusaDatabase::StorageFileNames & files)
{
	utils::shared_ptr<coreUI::CoreCursorChanger> cursorChanger(new coreUI::CoreCursorChanger);
	...
	try{
		...
		std::list<medusaDatabase::IHMDBRemoteContext::DownloadOperationPtr> downloads;
		...
		medusaDatabase::IHMDBRemoteContext::OperationPtr op(new medusaDatabase::MultipleFilesDownloadStoreAndStatusUpdate(downloads,
			storage,
...
medusaDatabase::StorageFileNames filterOutLocalFiles(const medusaDatabase::StorageFileNames & inputFiles,
	medusaDatabase::IHMDBStoragePtr storage)
{
	medusaDatabase::StorageFileNames ret;
...
void DataViewWidget::onDownload()
{
	...
	if (currentShallowCopy_ != nullptr && shallowCopyContext_->shallowCopyRemoteContext() != nullptr && sItems.empty() == false){
		...
		medusaDatabase::StorageFileNames files;
		...
		medusaDatabase::StorageFileNames toDownloadFiles = filterOutLocalFiles(files, shallowCopyContext_->shallowCopyLocalContext()->localContext()->dataContext()->storage());
...
void DataViewWidget::onDownloadAll()
{
	if (currentShallowCopy_ != nullptr && shallowCopyContext_->shallowCopyRemoteContext() != nullptr){
		...
		medusaDatabase::StorageFileNames files = ShallowCopyUtils::files(*currentShallowCopy_);
...
void DataViewWidget::onContextMenu(QPoint position)
{
	...
	if (selectedItems.empty() == true){
		...
		medusaDatabase::TreeWidgetContentItem * citem = dynamic_cast<medusaDatabase::TreeWidgetContentItem *>(item);
		if (citem != nullptr){
			medusaDatabase::DataType dt = FileType;
			...
			if (ok == true){
				...
				ui->actionLoad->setEnabled((ds.usage() & medusaDatabase::DataStatus::Unloaded) && (ds.storage() & medusaDatabase::DataStatus::Local));
				ui->actionUnload->setEnabled(ds.usage() & medusaDatabase::DataStatus::Loaded);
				ui->actionDownload->setEnabled(ds.storage() & medusaDatabase::DataStatus::Remote);
				ui->actionForceDownload->setEnabled(ds.storage() & medusaDatabase::DataStatus::Local);
			}
		}
...
medusaDatabase::StorageFileNames DataViewWidget::filterModifiedTrialFiles(IncrementalBranchShallowCopyConstPtr incShallowCopy)
{
...
{
	const auto status = transaction->dataStatus(medusaDatabase::MotionType, trial.trialID).storage();
	return (status == medusaDatabase::DataStatus::Local) || (status == medusaDatabase::DataStatus::PartiallyLocal);
}
