...
medusaDatabase::IHMDBShallowCopyContext * LayeredImageVisualizer::remoteShallowContext(const plugin::IVisualizer::ISerie * serie)
{
	if (serie != nullptr){
...
void dicom::LayeredImageVisualizer::uploadSerie()
{
	...
	if (correctIndex(currentSerie)) {
		...
		try{
			...
			medusaDatabase::IHMDBStorageOperations::IStreamPtr stream(new std::ifstream(p.string(), std::ios::binary));
			...
			auto upload = remote->prepareFileUpload(fileName, "/BDR/w", stream, medusaDatabase::IHMDBRemoteContext::Motion, fileSize);
			upload->start();
			upload->wait();
