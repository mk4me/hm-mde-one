...
void dicom::DicomService::update(double deltaTime)
{
	...
	//			auto comm = core::querySource<medusaDatabase::IHMDBSource>(plugin::getSourceManager());
...
const bool dicom::DicomService::userIsReviewer(medusaDatabase::IHMDBSession * session)
{
	static std::map<medusaDatabase::IHMDBSession *, bool> memoMap;
