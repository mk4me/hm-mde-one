...
void FilesHelper::getSessionSpecificFiles(int sessionID, const communication::ShallowCopy & shallowCopy, std::set<int> & files)
{
...
void FilesHelper::getSpecificFiles(const webservices::MotionShallowCopy::Session * session, std::set<int> & files)
{
...
unsigned long long FilesHelper::filesSize(const std::set<int> & filesIDs, const communication::ShallowCopy & shallowCopy)
{
	unsigned long long ret = 0;

	auto filesITEnd = shallowCopy.motionShallowCopy->files.end();

	for(auto it = filesIDs.begin(); it != filesIDs.end(); ++it){
		auto fileIT = shallowCopy.motionShallowCopy->files.find(*it);
		if(fileIT == filesITEnd){
			throw std::runtime_error("File does not exist in ShallowCopy");
		}

		ret += fileIT->second->fileSize;
	}

	return ret;
}

unsigned long long FilesHelper::fileSize(const int fileID, const communication::ShallowCopy & shallowCopy)
{
	std::set<int> files;
	files.insert(fileID);
	return filesSize(files, shallowCopy);
}
