/********************************************************************
	...
	purpose:
*********************************************************************/
#ifndef __HEADER_GUARD_medusaDatabase__DATAVIEWCONFIGURATIONWIDGET_H__
#define __HEADER_GUARD_medusaDatabase__DATAVIEWCONFIGURATIONWIDGET_H__
...
class medusaDatabase_EXPORT DataViewConfigurationWidget : public QWidget
{
	Q_OBJECT
...
public:
	...
	medusaDatabase::ShallowCopyFilter * currentFilter();
	//! \return Aktualna perspektywa danych - nie mo¿e byæ nullptr
	medusaDatabase::IDataSourcePerspective * currentPerspective();
	//! \return Aktualny content danych - nie mo¿e byæ nullptr
	medusaDatabase::IDataSourceContent * currentContent();

	//! \param filter Rejestrowany filtr
	void registerFilter(medusaDatabase::ShallowCopyFilter * filter);
	//! Dodaje przerwê miêdzy filtrami
	void addFilterSeparator();
	//! \param perspective Rejestrowana perspektywa
	void registerPerspective(medusaDatabase::IDataSourcePerspective * perspective);
	//! Dodaje przerwê miêdzy perspektywami
	void addPerspectiveSeparator();
	//! \param filter Rejestrowany content
	void registerContent(medusaDatabase::IDataSourceContent * content);
...
private:
	...
	//! Obs³uga filtrów
	utils::shared_ptr < ItemHelper<medusaDatabase::ShallowCopyFilter>> filters;
	//! Obs³uga perspektyw
	utils::shared_ptr < ItemHelper<medusaDatabase::IDataSourcePerspective>> perspectives;
	//! Obs³uga contentów
	utils::shared_ptr < ItemHelper<medusaDatabase::IDataSourceContent>> contents;
};

#endif	// __HEADER_GUARD_medusaDatabase__DATAVIEWCONFIGURATIONWIDGET_H__
