...
#include <plugins/medusaDatabase/OperationProgressWidget.h>
#include <plugins/medusaDatabase/IHMDBSourceViewManager.h>
#include <plugins/medusaDatabase/IDataSourcePerspective.h>
#include <corelib/PluginCommon.h>
#include <plugins/medusaDatabase/GeneralSourceViewWidget.h>
#include <plugins/medusaDatabase/DataViewWidget.h>
#include <plugins/medusaDatabase/DataViewConfigurationWidget.h>
...
#include "plugins/medusaDatabase/SourceOptionsWidget.h"
#include "plugins/medusaDatabase/ContextConfigurationSettingsFile.h"

using namespace core;

class MEDUSAHMDBSourceView : public medusaDatabase::IHMDBSourceViewManager::IHMDBSourceContextView
{
public:
	//! \return Nazwa widoku
	...
	virtual QWidget * createView(medusaDatabase::IHMDBShallowCopyContextPtr shallowCopyContext, medusaDatabase::IHMDBSourceViewManager * viewManager) {
		...
		medusaDatabase::IDataSourcePerspective * perspective = nullptr;
		medusaDatabase::IDataSourceContent * content = nullptr;
...
class MEDUSAPerspective : public medusaDatabase::IDataSourcePerspective
{
public:
	...
	virtual void rebuildPerspective(QTreeWidgetItem * treeWidgetItem, const medusaDatabase::ShallowCopy & shallowCopy)
	{
		std::multimap<hmdbServices::ID, medusaDatabase::TreeWidgetContentItem*> sessionItems;
		std::list<medusaDatabase::TreeWidgetContentItem*> unrecognizedSessionItems;
		...
		for (auto subjectIT = shallowCopy.motionShallowCopy.performers.begin(); subjectIT != subjectsITEnd; ++subjectIT){
			...
			for (auto perfConfIT = subjectIT->second->performerConfs.begin(); perfConfIT != perfConfsITEnd; ++perfConfIT){
				...
				//generuje item sesji
				auto sessionItem = new medusaDatabase::TreeWidgetContentItem(medusaDatabase::SessionContent, perfConfIT->second->session->sessionID);

				const auto sNumber = sessionLocalNumebr(perfConfIT->second->session->sessionName);

				if (sNumber > -1){
					sessionItems.insert(std::multimap<hmdbServices::ID, medusaDatabase::TreeWidgetContentItem*>::value_type(sNumber, sessionItem));
				}
				else{
				...
				for (auto motionIT = perfConfIT->second->session->trials.begin(); motionIT != motionsITEnd; ++motionIT){
					...
					auto motionItem = new medusaDatabase::TreeWidgetContentItem(medusaDatabase::MotionContent, motionIT->second->trialID);
...
bool MdeMainWindow::customViewInit(QWidget * log)
{
...
#endif
   utils::shared_ptr<medusaDatabase::IHMDBSource> icomm = core::querySource<medusaDatabase::IHMDBSource>(plugin::getSourceManager());
   ...
   auto vm = icomm->viewManager();
	...
	vm->registerContent(new medusaDatabase::DataSourceDefaultContent, hmdbView->name());
...
medusaDatabase::IHMDBSourceViewManager::ContextConfiguration MdeMainWindow::createHmdbConfig()
{
	medusaDatabase::IHMDBSourceViewManager::ContextConfiguration ccfg;
	...
	typedef medusaDatabase::ContextConfigurationSettingsFile ConfFile;
