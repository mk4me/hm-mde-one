	...
	Qt::WindowFlags f) : QWidget(parent, f), ui(new Ui::DataViewConfigurationWidget)
{
	...
	filters.reset(new ItemHelper<medusaDatabase::ShallowCopyFilter>(ui->filterLabel, ui->filterComboBox));
	perspectives.reset(new ItemHelper<medusaDatabase::IDataSourcePerspective>(ui->perspectiveLabel, ui->perspectiveComboBox));
	contents.reset(new ItemHelper<medusaDatabase::IDataSourceContent>(ui->contentLabel, ui->contentComboBox));
...
medusaDatabase::ShallowCopyFilter * DataViewConfigurationWidget::currentFilter()
{
	return filters->currentItem();
}

medusaDatabase::IDataSourcePerspective * DataViewConfigurationWidget::currentPerspective()
{
	return perspectives->currentItem();
}

medusaDatabase::IDataSourceContent * DataViewConfigurationWidget::currentContent()
{
	return contents->currentItem();
}

void DataViewConfigurationWidget::registerFilter(medusaDatabase::ShallowCopyFilter * filter)
{
	filters->add(filter);
}
...
void DataViewConfigurationWidget::registerPerspective(medusaDatabase::IDataSourcePerspective * perspective)
{
...
void DataViewConfigurationWidget::registerContent(medusaDatabase::IDataSourceContent * content)
{
