...
#include <plugins/medusaDatabase/ServerStatusWidget.h>
...
void SourceOptionsWidget::onRefreshConfigurations()
{
	...
	if (sm != nullptr){
		auto hmdbSource = core::querySource<medusaDatabase::IHMDBSource>(sm);
		...
		if (sm != nullptr){
			...
			if (hmdbSource != nullptr){
				...
				if (vm->viewPrototypesCount() > 0
					...
					medusaDatabase::IHMDBSourceViewManager::IHMDBSourceContextView * view = nullptr;
...
void SourceOptionsWidget::setConnectionProfile(const medusaDatabase::IHMDBSourceViewManager::ContextConfiguration & connectionProfile)
{
...
void SourceOptionsWidget::onLogin()
{
	...
	auto hmdbService = core::queryService<medusaDatabase::IHMDBService>(sm);	
	auto srcm = plugin::getSourceManager();	
	auto hmdbSource = core::querySource<medusaDatabase::IHMDBSource>(srcm);	

	QString message;

	try{

		utils::shared_ptr<medusaDatabase::SQLCipherStorage> storage(new medusaDatabase::SQLCipherStorage);

		storage->open(ui->storagePathLineEdit->text().toStdString(),
			ui->storagePasswordLineEdit->text().toStdString());

		medusaDatabase::IHMDBSessionPtr session;
...
const bool SourceOptionsWidget::verify(QStringList & messages)
{
	...
	utils::shared_ptr<medusaDatabase::IHMDBService> hmdbService;
	if (sm != nullptr){
		hmdbService = core::queryService<medusaDatabase::IHMDBService>(sm);
		if (hmdbService == nullptr){
			messages.push_back(tr("HMDBService not found"));
		}
	}

	utils::shared_ptr<medusaDatabase::IHMDBSource> hmdbSource;
	if (srcm != nullptr){
		hmdbSource = core::querySource<medusaDatabase::IHMDBSource>(srcm);
		if (hmdbSource == nullptr){
	...
	if (storagePath.empty() == true){
		...
		if (ret == QMessageBox::Yes){
			...
			else if (medusaDatabase::SQLCipherStorage::create(storagePath, ui->storagePasswordLineEdit->text().toStdString()) == true){
	...
	if (storageCreated == false && utils::Filesystem::pathExists(storagePath) == true) {
		std::unique_ptr<medusaDatabase::SQLCipherStorage> storage(new medusaDatabase::SQLCipherStorage);
...
void SourceOptionsWidget::verify(hmdbServices::IFileStoremanWS * fileStoreman,
	medusaDatabase::IHMDBFtp * ftp, const QString & errorMessage,
	QStringList & errors)
{
...
const medusaDatabase::IHMDBSourceViewManager::ContextConfiguration& SourceOptionsWidget::getConnectionProfile() const
{
