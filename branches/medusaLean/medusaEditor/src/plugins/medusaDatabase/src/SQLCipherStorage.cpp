...
class SQLCipherStorageTransaction : public threadingUtils::TransactionImplHelper<medusaDatabase::IHMDBStorageOperations, SQLCipherStorage::TransactionSharedState>
{
private:
...
public:
	...
	virtual void set(const std::string & key, IStreamPtr input, medusaDatabase::IHMDBStorageProgress * progress) override
	{
...
private:
	...
	static void rawSet(const std::string & key, IHMDBStorageOperations::IStreamPtr input, medusaDatabase::IHMDBStorageProgress * progress, sqlite3 * db)
	{
		if (input == nullptr){
