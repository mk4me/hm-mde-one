...
{
	auto icomm = core::querySource<medusaDatabase::IHMDBSource>(plugin::getSourceManager());
	if (icomm) {
		...
		if (utils::Filesystem::isDirectory(dir)) {
			...
			const medusaDatabase::IHMDBShallowCopyContextPtr context =  icomm->shallowContext(0);
			...
			for (auto file : files) {
				...
				if (storage->exists(name)) {
					const medusaDatabase::IHMDBStorageOperations::IStreamPtr stream = storage->get(name);
					if (stream) {
