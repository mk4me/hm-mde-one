    ...
    :   service(service)
{
    ...
    //dla lapania pozycji kursora na menu kontekstowe ponieważ z poziomu OSG tracę częściowo tą informację
    ...
    for(int i = 0; i < dp->getNumPrototypes(); ++i){
        //opakuj prototyp w fabrykę węzła dla EDRDataFlow



        //dodaj odpowiadający element do toolbara
    }
...
void WorkflowCustomQOSGWidget::tryAddNode()
{
    try{
        ...
        if(dynamic_cast<InputDescription*>(item.get()) == nullptr){
            //sprawdz czy węzeł poprawnie zbudowany!!
...
void WorkflowCustomQOSGWidget::mousePressEvent(QMouseEvent * event)
{
    if(event->button() == Qt::LeftButton){
        ...
        //prawy klawisz wiec może chce usunąć połączenia
...
WorkflowItemPtr EDRWorkflowWidget::buildAndInitializeVisualizer(UniqueID id)
{
    ...
    ////nie niszcz wizualizatora przy jego zamykaniu !! dopiero usunięcie węzła powinno to robić
    visWidget->setPermanent(true);

	supplyWithEDRTitleBar(visWidget);

    //dodajemy wizualizator do głównego okna
