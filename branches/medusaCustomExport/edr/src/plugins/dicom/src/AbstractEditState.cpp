...
QGraphicsItem* dicom::AbstractEditState::extractItem(QGraphicsSceneMouseEvent* e)
{
	...
	for (auto& i : items) {
		if (i != &line[0] && i != &line[1]) {
			bool test1 = i->boundingRect().contains(e->scenePos());
			bool test2 = i->sceneBoundingRect().contains(e->scenePos());
			return i;
		}
	}
...
QGraphicsItem* dicom::AbstractEditState::extractItem(QGraphicsSceneMouseEvent* e, const PointsLayer& pl)
{
	auto items = machine->getGraphicsScene()->items(e->scenePos());

	for (auto& i : items) {
		if (pl.hasPoint(i)) {
			auto gv = machine->getGraphicsView();
			auto rect = i->boundingRect();
			auto brect = i->sceneBoundingRect();
			auto t = gv->transform();
			
			
			auto cx = brect.center().x();
			auto cy = brect.center().y();
			auto scale = sqrt(t.m11()*t.m11() + t.m22() * t.m22() + t.m33() * t.m33());
			auto w = brect.width() / scale;
			auto h = brect.height() / scale;
			brect = QRectF(cx - w/2, cy - h/2, w, h);
			if (brect.contains(e->scenePos())) {
				return i;
			}
		}
	}
