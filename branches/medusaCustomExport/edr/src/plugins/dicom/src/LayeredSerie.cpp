...
void trySetWhitePenAndBrush(QGraphicsItem* itm)
{
	QGraphicsPathItem* pitm = dynamic_cast<QGraphicsPathItem*>(itm);
	if (pitm) {
		pitm->setBrush(QBrush(Qt::white));
		pitm->setPen(QPen(Qt::white));
	}

	auto items = itm->childItems();
	for (auto i : items) {
		trySetWhitePenAndBrush(i);
	}
}

QImage createSynovitisWhites(const QPixmap& pixmap, const std::vector<dicom::ILayerGraphicItemConstPtr>& v)
{
	...
	for (auto it = v.begin(); it != v.end(); ++it) {
		dicom::ILayerGraphicItemConstPtr graphic = *it;
		if (graphic) {
			dicom::ILayerGraphicItemPtr cln(graphic->clone());
			QGraphicsItem* itm = cln->getItem();
			trySetWhitePenAndBrush(itm);
			scene.addItem(itm);
...
void dicom::LayeredSerie::setExtractBackground(const QString& path)
{
	auto imgL = getImage();
	QImage p(imgL->getSize(), QImage::Format_RGB32);
	p.fill(Qt::black);

	std::string user = getCurrentLayerUserName();
	int num = getImage()->getNumGraphicLayerItems(user);
	std::vector<dicom::ILayerGraphicItemConstPtr> layers;
	for (int i = 0; i < num; ++i) {
		auto layer = getImage()->getLayerGraphicItem(user, i);
		if (layer->getAdnotationIdx() == dicom::annotations::inflammatory) {
			layers.push_back(layer);
		}
	}
	QImage result = createSynovitisWhites(QPixmap::fromImage(p), layers);
	result.save(path);
}
