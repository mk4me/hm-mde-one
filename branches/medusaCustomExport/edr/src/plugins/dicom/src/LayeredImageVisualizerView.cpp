...
rejectAction(nullptr), toVerifyAction(nullptr)
{
	...
	QIcon setExportBackgroundPathIcon = QIcon(":/coreUI/icons/open.png");
	coreUI::CoreAction*  setExportPathActions = new coreUI::CoreAction(tr("Other"), setExportBackgroundPathIcon, tr("Set Export Background Path"), this, coreUI::CoreTitleBar::Left);
	connect(setExportPathActions, SIGNAL(triggered()), this, SLOT(setExportPath()));
	QIcon exportBackgroundIcon = QIcon(QStringLiteral(":/coreUI/icons/x.png"));
	coreUI::CoreAction* exportBackgroundAction = new coreUI::CoreAction(tr("Other"), exportBackgroundIcon, tr("Export Background"), this, coreUI::CoreTitleBar::Left);
	connect(exportBackgroundAction, SIGNAL(triggered()), this, SLOT(exportBackground()));

	this->addAction(setExportPathActions);
	this->addAction(exportBackgroundAction);
...
void dicom::LayeredImageVisualizerView::setExportPath()
{
	QSettings settings;
	QString path = settings.value("exportSynovitisDir").toString();
	path = QFileDialog::getExistingDirectory(0, QString(), path);
	QDir dir(path);
	if (dir.exists()) {
		settings.setValue("exportSynovitisDir", path);
	}
}

void dicom::LayeredImageVisualizerView::exportBackground()
{
	LayeredSerie* serie = dynamic_cast<LayeredSerie*>(model->getActiveSerie());
	if (serie) {
		QSettings settings;
		QString path = settings.value("exportSynovitisDir").toString();
		std::string ppp = path.toStdString();
		QDir dir(path);
		if (dir.exists(path)) {
			std::string filename = serie->getImageFilename();
			std::string basePrefix = "sqlcipher://";
			if (filename.find(basePrefix) == 0) {
				filename = filename.substr(basePrefix.length());
			}
			utils::Filesystem::Path p(filename);
			filename = p.stem().string();
			filename += ".";
			filename += serie->getLoggedUserName();
			filename += ".mask.png";
			serie->setExtractBackground(dir.filePath(QString::fromStdString(filename)));
		}
		else {
			QMessageBox::warning(this, tr("Problem"), QString("Directory : \"%1\" does not exists").arg(path));
		}
	}
}
