...
void dicom::EditState::begin( coreUI::AbstractStateConstPtr lastState )
{
	...
	for (auto tn = 0; tn < image->getNumTags(); ++tn) {
		auto tag = image->getTag(tn);//.toStdString();
		for (int i = 0; i < image->getNumGraphicLayerItems(tag); i++) {
			auto l = image->getLayerGraphicItem(tag, i);
			l->getItem()->setFlag(QGraphicsItem::ItemIsMovable, false);
			l->getItem()->setFlag(QGraphicsItem::ItemIsSelectable, false);
		}
	}

	for (int i = 0; i < layer->getNumPoint(); ++i) {
		auto p = layer->getPoint(i);
...
void dicom::EditState::end()
{
	...
	for (auto tn = 0; tn < image->getNumTags(); ++tn) {
		...
		for (int i = 0; i < image->getNumGraphicLayerItems(tag); i++) {
			auto l = image->getLayerGraphicItem(tag, i);
			l->getItem()->setFlag(QGraphicsItem::ItemIsMovable, true);
			l->getItem()->setFlag(QGraphicsItem::ItemIsSelectable, true);
		}
	}
}
