...
#include "../../dicom/src/PointsLayer.h"
...
void medusaExporter::ExporterModel::exportData(const QString& outDir, const QString& user, const QString& dirPath, const ExportConfig& config, CallbackFunction fun)
{
	...
	if (config.separateDifferent) {
		separateDifferentImages(outDir, *annotations, fun);
	}
...
void medusaExporter::ExporterModel::separateDifferentImages(const QString& outDir, AnnotationData& data, CallbackFunction fun)
{
	QString subDir = outDir + "/separated";
	QDir od(subDir);
	bool makedir = od.mkdir(".");
	...
	for (auto itLayer = layers.begin(); itLayer != layers.end(); ++itLayer) {
		...
		for (auto it = itLayer->second.begin(); it != itLayer->second.end(); ++it) {
			auto imgT = boost::dynamic_pointer_cast<const dicom::IValueLayer>(tryGetAnnotation(itLayer->second, dicom::annotations::imageType));
			std::string imageType = imgT ? imgT->valueAsString() : "-1";
			if (imageType == "different") {
				auto filePath = QString("%1/Data/%2").arg(outDir).arg(filename);
				auto destPath = QString("%1/%2").arg(subDir).arg(filename);
