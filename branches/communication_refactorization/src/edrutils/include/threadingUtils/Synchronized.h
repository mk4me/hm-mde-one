/********************************************************************
	created:  2011/05/12
	created:  12:5:2011   15:21
	filename: Synchronized.h
	author:   Mateusz Janiak

	purpose:  Klasa do zapewniania synchronizacji na poziomie obiektów.
	By wykonać operacje na obiekcie należy wykonać na nim LOCK(x) { blok instrukcji }
	lub posłużyc się typem lokującym dany obiekt na czas swojego życia - _Locker

	Koncepcja pochodzi ze storny: http://tlzprgmr.wordpress.com/2008/08/10/adding-a-lock-statement-to-c/
	*********************************************************************/
#ifndef HEADER_GUARD_THREADINGUTILS__SYNCHRONIZED_H__
#define HEADER_GUARD_THREADINGUTILS__SYNCHRONIZED_H__

#include <boost/type_traits/is_scalar.hpp>
#include <boost/mpl/if.hpp>
#include <threadingUtils/SynchronizationPolicies.h>

namespace threadingUtils
{
	template<bool threading, typename SyncPolicyT = RecursiveSyncPolicy>
	class SynchronizedT : public boost::mpl::if_c<threading, SyncPolicyT, NoSyncPolicy>::type
	{
	public:

		typedef typename boost::mpl::if_c<threading, SyncPolicyT, NoSyncPolicy>::type SyncPolicy;
		typedef typename boost::mpl::if_c<threading, ScopedLock<SyncPolicyT>, EmptyScopedLock>::type ScopedLock;

	protected:

		SynchronizedT()
		{
			mux = this;
		}

	public:

		virtual ~SynchronizedT() {}

	protected:

		mutable RecursiveSyncPolicy * mux;
	};

	template<class T, bool>
	class SynchronizedImpl : public T
	{
	public:
		SynchronizedImpl() : T() { }
		SynchronizedImpl(const T &x) : T(x) { }
	};

	template<class T>
	class SynchronizedImpl<T, true>
	{
	public:
		SynchronizedImpl() { }
		SynchronizedImpl(const T &x) : x_(x) { }
		operator T &() { return x_; }
		operator const T &() const { return x_; }
		SynchronizedImpl &operator= (T x) { x_ = x; return *this; }

	private:
		T x_;
	};

	template<class T, class SynchPolicy>
	class Synchronized :
		public SynchronizedImpl<T, boost::is_scalar<T>::value>, public SynchPolicy
	{
	private:
		typedef SynchronizedImpl<T, boost::is_scalar<T>::value> base;

	public:
		Synchronized() : base() { }
		explicit Synchronized(const T &x) : base(x) { }

		Synchronized &operator= (const T &rhs)
		{
			base &b = *this;
			b = rhs;
			return *this;
		}
	};

	template<class T>
	class _Locker {
	public:

		_Locker(T& m) : m_(m) { m_.lock(); run_ = true; }
		_Locker(T& m, int) : m_(m) { run_ = m_.try_lock(); }
		~_Locker() { m_.unlock(); }
		bool _run() { return run_; }
		void _stop() { run_ = false; }
	private:
		T&   m_;
		bool            run_;
	};

#define LOCK(x) for (_Locker _lock(x); _lock._run(); _lock._stop())
#define TRY_LOCK(x) for (_Locker _lock(x, 0); _lock._run(); _lock._stop())
}

#endif  //  HEADER_GUARD_THREADINGUTILS__SYNCHRONIZED_H__
