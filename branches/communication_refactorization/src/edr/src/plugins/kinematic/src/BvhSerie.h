...
private:
	typedef std::map<kinematic::JointConstPtr, int> IDMap;

private:
	void traverse(kinematic::JointConstPtr joint, const osg::Matrix & parentMat, std::vector<osg::Vec3> & positions);
	...
	IDMap generateIds(const kinematic::Skeleton& skeleton);
	SegmentsDescriptors generateConnections(const kinematic::Skeleton& skeleton, const IDMap& idmap);
	osg::Quat getJointCurrentLocalRotation(kinematic::JointConstPtr joint);
	osg::Vec3 getCurrentRootPosition(kinematic::JointConstPtr root);
private:
