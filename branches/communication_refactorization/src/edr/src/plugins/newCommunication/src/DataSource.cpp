...
void CommunicationDataSource::setCurrentUser(const User & user)
{
	...
	userIsReviewer_ = (ug.find(11) != ug.end() || ug.find(12) != ug.end());
...
{
	hmdbServices::MotionShallowCopy::Performer * sub = nullptr;
	hmdbServices::MedicalShallowCopy::Patient * pat = nullptr;
	...
	QMetaObject::invokeMethod(dataSourceWidget, "setPatientCard", Q_ARG(hmdbServices::MedicalShallowCopy::Patient *, pat),
		Q_ARG(hmdbServices::MotionShallowCopy::Performer *, sub));
...
void CommunicationDataSource::extractFileFromLocalStorageToUserSpace(const hmdbServices::MotionShallowCopy::File * file, const std::string & sessionName)
{
...
bool CommunicationDataSource::uploadMotionFile(const core::Filesystem::Path& path, const std::string& trialName)
{
	...
	for (auto it = trials.begin(); it != trials.end(); ++it) {
		hmdbServices::MotionShallowCopy::Trial* trial = it->second;
	...
	for (auto it = files.begin(); it != files.end(); ++it) {
		hmdbServices::MotionShallowCopy::File* file = it->second;
...
{
	/*hmdbServices::IncrementalBranchShallowCopy cpy;
	hmdbServices::IncrementalBranchShallowCopyParser::parseFile("E:/Incremental.xml", cpy);
...
hmdbServices::IncrementalBranchShallowCopy CommunicationDataSource::getIncrementalShallowCopy(const hmdbServices::DateTime& dt)
{
...
const hmdbServices::xmlWsdl::AnnotationsList CommunicationDataSource::getAllAnnotationStatus() const
{
...
const hmdbServices::xmlWsdl::UsersList CommunicationDataSource::listUsers() const
{
	auto ws = DataSourceWebServicesManager::instance();
	auto res = ws->authorizationService()->listUsers();
	return hmdbServices::xmlWsdl::parseUsersList(res);
}
	...
	const std::string & comment)
{
	...
	if (userIsReviewer_ == true){
		...
		if (type == hmdbServices::xmlWsdl::AnnotationStatus::Approved ||
			...
			ws->motionBasicUpdatesService()->setAnnotationReview(motionID, userID, hmdbServices::xmlWsdl::AnnotationStatus::UnderReview, comment);
		}
...
const hmdbServices::DateTime CommunicationDataSource::lastUpdateTime() const
{
	hmdbServices::DateTime ret;
...
void CommunicationDataSource::refreshAnnotationStatus()
{
	...
	if (userIsReviewer_ == false){
		...
		annotations = hmdbServices::xmlWsdl::parseAnnotations(myAnnotationsRes);
	}
	else{
		//lekarz
		std::string annotationsRes = ws->motionBasicQueriesService()->listAnnotationsXML();
		annotations = hmdbServices::xmlWsdl::parseAnnotations(annotationsRes);
	}
}
