/********************************************************************
	...
	purpose:
	*********************************************************************/
#ifndef HEADER_GUARD_HMDBCOMMUNICATION__DATASOURCEWIDGET_H__
#define HEADER_GUARD_HMDBCOMMUNICATION__DATASOURCEWIDGET_H__
...
class DataSourceWidget : public QTabWidget, private Ui::DataSourceWidget
{
	Q_OBJECT;	
...
public:
	...
	DataSourceWidget(ShallowCopyFilterManager* fm,
	...
	void setPatientCard(hmdbServices::MedicalShallowCopy::Patient * patient, hmdbServices::MotionShallowCopy::Performer * subject);
...
private:
	...
	static utils::shared_ptr<hmdbCommunication::AntropometricData> createAntropometricData(const hmdbServices::MotionShallowCopy::Attrs & attrs);
	static float getAntropometricValue(const std::string & attribute, const hmdbServices::MotionShallowCopy::Attrs & attrs, float defValue = 0.0);
	...
	static void getItemsFiles(QTreeWidgetItem * item, std::set<int> & filesIDs, const hmdbCommunication::ShallowCopy & shallowCopy);
	static void getAdditionalItemsFiles(QTreeWidgetItem * item, std::set<int> & filesIDs);
	static void getPatientAndSubject(QTreeWidgetItem * item, const hmdbServices::MedicalShallowCopy::Patient *& patient,
		const hmdbServices::MotionShallowCopy::Performer *& subject);

	static QIcon statusIcon(const hmdbCommunication::DataStorage storage, const hmdbCommunication::DataUsage usage);
	...
	void registerPatientCard(hmdbCommunication::IPatientCard * patientCard);
	...
	virtual void update(const hmdbCommunication::IDownloadRequest * request);
...
private:
	...
	hmdbCommunication::ShallowCopy filteredShallowCopy;
	...
	hmdbServices::DateTime lastSynchroTime;
};

#endif	//	HEADER_GUARD_HMDBCOMMUNICATION__DATASOURCEWIDGET_H__
